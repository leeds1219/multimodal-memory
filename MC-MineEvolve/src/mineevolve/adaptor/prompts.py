...
def render_adaptor_user_prompt(
    ...
    return (
        ...
        f"<current_plan>\n"
        f"{json.dumps([dict(p) for p in current_plan], ensure_ascii=False, separators=(',',':'))}\n"
        f"</current_plan>\n\n"
        f"<frozen_prefix_ids>{list(frozen_prefix_ids)}</frozen_prefix_ids>\n\n"
        f"<recent_feedback count={len(recent_feedback)}>\n"
        f"{json.dumps([dict(e) for e in recent_feedback], ensure_ascii=False, separators=(',',':'))}\n"
        f"</recent_feedback>\n\n"
        ...
        f"{json.dumps(subgoal_level, ensure_ascii=False, separators=(',',':'))}\n"
        f"</active_remedies_subgoal_level>\n\n"
        f"<active_remedies_task_level_missing_prerequisite count={len(task_level_missing_prereq)}>\n"
        f"{json.dumps(task_level_missing_prereq, ensure_ascii=False, separators=(',',':'))}\n"
        ...
        f"{json.dumps(task_level_deadlock, ensure_ascii=False, separators=(',',':'))}\n"
        ...
        f"{json.dumps(task_level_recovery, ensure_ascii=False, separators=(',',':'))}\n"
